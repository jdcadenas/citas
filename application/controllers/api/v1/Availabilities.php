...
class Availabilities extends API_V1_Controller {
    ...
    public function get() {
        try {
            ...
            $emptyPeriods = $this->_getProviderAvailableTimePeriods($providerId->get(), $date->format('Y-m-d'), []);

            $availableHours = $this->_calculateAvailableHours($emptyPeriods, $date->format('Y-m-d'), $service['duration'], FALSE, $service['availabilities_type']);
    ...
    $provider_id, $selected_date, $exclude_appointments = []
    ) {
        $this->load->model('appointments_model');
        $this->load->model('providers_model');
