...
class Providers_Model extends CI_Model {
    ...
    public function validate($provider) {
        ...
        if (isset($provider['id'])) {
            $num_rows = $this->db->get_where('ea_users', ['id' => $provider['id']])->num_rows();
            if ($num_rows == 0) {
        ...
        if (!isset($provider['last_name']) || !isset($provider['email']) || !isset($provider['phone_number'])) {
        ...
        if (!isset($provider['settings']) || count($provider['settings']) == 0 || !is_array($provider['settings'])) {
        ...
        if (isset($provider['settings']['calendar_view']) && ($provider['settings']['calendar_view'] !== CALENDAR_VIEW_DEFAULT && $provider['settings']['calendar_view'] !== CALENDAR_VIEW_TABLE)) {
    ...
    public function get_row($provider_id) {
        ...
        $services = $this->db->get_where('ea_services_providers', ['id_users' => $provider_id])->result_array();
        $provider['services'] = [];
        ...
        $provider['settings'] = $this->db->get_where('ea_user_settings', ['id_users' => $provider_id])->row_array();
    ...
    public function validate_username($username, $user_id) {
        $num_rows = $this->db->get_where('ea_user_settings', ['username' => $username, 'id_users <> ' => $user_id])->num_rows();
