...
class Admins_Model extends CI_Model {
    ...
    public function validate($admin) {
        ...
        if (!isset($admin['last_name']) || !isset($admin['email']) || !isset($admin['phone_number'])) {
        ...
        if (isset($admin['settings']['calendar_view']) && ($admin['settings']['calendar_view'] !== CALENDAR_VIEW_DEFAULT && $admin['settings']['calendar_view'] !== CALENDAR_VIEW_TABLE)) {
    ...
    public function delete($admin_id) {
        ...
        $admin_count = $this->db->get_where('ea_users', ['id_roles' => $this->get_admin_role_id()])->num_rows();
        if ($admin_count == 1) {
    ...
    public function get_row($admin_id) {
        ...
        $admin['settings'] = $this->db->get_where('ea_user_settings', ['id_users' => $admin_id])->row_array();
    ...
    public function get_batch($where_clause = '') {
        ...
        foreach ($batch as &$admin) {
            $admin['settings'] = $this->db->get_where('ea_user_settings', ['id_users' => $admin['id']])->row_array();
            unset($admin['settings']['id_users']);
        }
